...
def agents_train(arglist, game_step, update_cnt, memory, obs_size, action_size, \
                actors_cur, actors_tar, critics_cur, critics_tar, optimizers_a, optimizers_c,mae_num):
    """ 
    ...
    if game_step > arglist.learning_start_step and \
        ...
        for agent_idx, (actor_c, actor_t, critic_c, critic_t, opt_a, opt_c) in \
            ...
            mae_index = [i for i in range(len(actors_cur)) if i!=agent_idx]
            mae_index = random.sample(mae_index,mae_num)
            ...
            for mae_agent_index in mae_index:
                obs_n_o[:, obs_size[mae_agent_index][0]:obs_size[mae_agent_index][1]] = 0
                obs_n_n[:, obs_size[mae_agent_index][0]:obs_size[mae_agent_index][1]] = 0
                # action_cur_o[:, action_size[mae_agent_index][0]:action_size[mae_agent_index][1]] = 0
        ...
        if update_cnt > arglist.start_save_model and update_cnt % arglist.fre4save_model == 0:
            ...
            print('=time:{}a step:{}        sve'.format(time_now, game_step))
            model_file_dir = os.path.join(arglist.save_dir, '{}_{}_{}'.format( \
                arglist.scenario_name+"SMPG", time_now, game_step))
        ...
        arglist.tao = 1-nn.Tanh()((torch.abs(q)).mean()/(torch.abs(tar_value)).mean()).detach().cpu()
        # update the tar par
...
def train(arglist):
    ...
    mae_ratio = arglist.mae_ratio
    mae_num = int((env.n-1)*mae_ratio)
    ...
    lr_scheduler_a = []
    lr_scheduler_c = []
    for i in range(len(optimizers_a)):
        lr_scheduler_a.append(optim.lr_scheduler.StepLR(optimizers_a[i], step_size = 100, gamma = 0.99))
    for i in range(len(optimizers_c)):
        lr_scheduler_c.append(optim.lr_scheduler.StepLR(optimizers_c[i], step_size = 100, gamma = 0.99))
    ...
    for episode_gone in range(arglist.max_episode):
        # call the reward print the debug data
        ...
        for episode_cnt in range(arglist.per_episode_max_len):
            ...
            if done or terminal:
                ...
                continue
        for i in range(len(optimizers_a)):
            lr_scheduler_a[i].step()
            lr_scheduler_c[i].step()
